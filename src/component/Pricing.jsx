        ...
        {pricingOptions.map((option, index) => (
          <div key={index} className="w-full sm:w-1/2 lg:w-1/3 p-2">
            <div className="p-10 border border-neutral-700 rounded-xl">
              ...
              <ul className="">
                {option.features.map((featurs, index) => (
                  <li key={index} className="mt-8 flex items-center ">
                    <CheckCircle2 className="inline-block mr-2 text-green-500" />
                    <span className="ml-2">{featurs}</span>
                  </li>
                ))}
              </ul>
              <a
                href="#"
                className="inline-flex justify-center items-center text-center w-full h-12 p-5 mt-20 tracking-tight
